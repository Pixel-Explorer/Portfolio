...
export function createArchiveTerrain(options) {
  ...
  // Pass Final: reflector ON by default for city reflection. Opt out via
...
if (!CLUSTER_MODE) {
  ...
  //   azimuth ≈ 0
  const camTarget = new THREE.Vector3(0, 18.0, 0);
  ...
  const STAGER_BUILDING_ENTRY = {
    ...
    "Octo Research": { cluster: true, label: "Research & Development", entryIds: [68, 69, 140] },
    "map oIl": 123,
    ...
    "Movies": { cluster: true, label: "Movies & Film", entryIds: [42, 46, 121, 122, 84, 65, 20, 136, 139] },
    "Travel Film": { cluster: true, label: "Travel Films", entryIds: [50, 43, 94, 141] },
  ...
  function resetView() {
    ...
    const targetY = CLUSTER_MODE ? 18.0 : 0.5;
    const targetR = CLUSTER_MODE ? 115.0 : gridWidth * 1.65;
    const targetPolar = CLUSTER_MODE ? Math.PI * 0.44 : Math.PI * 0.34;
