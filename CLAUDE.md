...
**Pass 2 (BRUTAL-FOLDER-SPEC.md) shipped:** reflow fix for folder bottom-slide (force reflow before `.visible`); collapsed height 46vh; cut folder tab; rich peek (title + tags + thumb + 3-line story); clusters = stacked multi-colored folder cards with staggered tabs; tighter camera framing.
