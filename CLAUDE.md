...
- **CODEX is an indrajaal-museum-style big-type list** (`initCodexScroller`): huge centered uppercase titles in a `.codex-track` rendered TWICE for a seamless infinite loop, driven by a custom transform scroller with drag + wheel + momentum (native scroll can't loop). `codex-active` on the overlay makes `.gallery-viewport` stop native-scrolling. No inline thumbnails — hovering a row floats the cursor-trailing preview. A drag (>6px) sets `codexJustDragged` to suppress the row click. NOTE: do NOT `setPointerCapture` on the scroller — it steals the row's click and the artifact never opens.
