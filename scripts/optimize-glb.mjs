...
export async function optimizeGlb(input, outputPath = null, opts = {}) {
  ...
  const { textureSize = 2048, meshoptCompress = true, quality = 85, textureFormat = "webp" } = opts;
...
if (isCli) {
  ...
  if (!inputPath) {
    console.error("usage: node scripts/optimize-glb.mjs <in.glb> [out.glb] [--size 2048] [--no-meshopt]");
    process.exit(1);
  }
  ...
  console.log(`  input: ${mb(beforeBytes)} | textures → ${textureSize}px | meshopt: ${meshoptCompress}`);
  const t0 = Date.now();
  const { after } = await optimizeGlb(inputPath, outputPath, { textureSize, meshoptCompress, textureFormat });
