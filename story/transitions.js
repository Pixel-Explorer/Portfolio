export class Transitions {
  ...
  destroy() {
    if (this._gsap) this._gsap.killTweensOf(this._overlay);
  ...
  async drain(duration = 0.8) {
    if (this._reduceMotion) { this._setOverlay('#000000', 0.5, 'none'); await this._wait(0.1); this._setOverlay('#000000', 0, 'none'); return; }
  ...
  async break(duration = 0.5) {
    if (this._reduceMotion) { this._setOverlay('#000000', 1, 'none'); await this._wait(0.2); this._setOverlay('#000000', 0, 'none'); return; }
    const seq = this._transitionSeq;
  ...
  async bloom(duration = 0.7) {
    if (this._reduceMotion) { this._setOverlay('#ffffff', 0.6, 'none'); await this._wait(0.1); this._setOverlay('#ffffff', 0, 'none'); return; }
    const seq = this._transitionSeq;
