...
3. `StoryEngine` - 23 edges
4. `init()` - 22 edges
5. `AudioManager` - 20 edges
6. `StoryUI` - 17 edges
7. `Orb` - 16 edges
8. `TASKS` - 15 edges
9. `Advanced Web Interaction Patterns — Study Report` - 15 edges
...
- `mergeEntries()` --calls--> `uniq()`  [INFERRED]
  bin/scripts/duplicate_cleanup.mjs → bin/scripts/clean_ledger.mjs
...
Cohesion: 0.26
Nodes (5): BEATS, ERA_COLORS, BG_COLORS, BEAT_TUNING, GLOBAL_TUNING
...
Cohesion: 0.11
Nodes (11): files, io, mats, mtlName, mtlPath, mtlText, objName, objPath (+3 more)
...
Cohesion: 0.20
Nodes (15): caseStudiesPath, handleApi(), ledgerPath, loadCaseStudies(), loadLedger(), port, readJsonBody(), readRawBody() (+7 more)
...
Cohesion: 0.19
Nodes (13): initGridCanvas(), bounds, decompose(), IDENT, io, len3(), mul(), results (+5 more)
...
Cohesion: 0.19
Nodes (14): closeArtifactView(), getVisibleEntries(), groupBy(), initApp(), initTerrain(), loadLedgerData(), resetPageSEO(), selectEmptyWeek() (+6 more)
...
Cohesion: 0.12
Nodes (15): 0. OpenCode kickoff prompt (paste this to start), 1. Goal, 2. Architecture facts you must know, 3. The proven pattern (copy this), 4.1 Inline styles set by JS → KEEP `!important`, 4.2 Higher-specificity legacy competitor → keep `!important` OR delete the legacy rule, 4.3 Measure with transitions SUPPRESSED, 4. CRITICAL gotchas (each one bit during the reference work) (+7 more)

### Community 189 - "Community 189"
Cohesion: 0.13
Nodes (14): §1. What you're building (the core ask), §2. Files & exact reuse targets, §3. Codex view spec (the stub to replace), §4. Smaller open items (do if time; don't block on them), §5. Data schema — FINAL, do not migrate, §6. Acceptance, §7. Run / verify, §8. Anti-asks (+6 more)
...
Cohesion: 0.39
Nodes (8): detectLinkType(), evidenceToSlot(), extractBehanceId(), extractGoogleDriveId(), extractInstagramPath(), extractXPostPath(), extractYouTubeId(), googleDocPreview()
...
Cohesion: 0.18
Nodes (10): Context, Conventions (MUST follow), Files Changed Summary, Item 1: Magnetic Hover on Interactive Elements, Item 2: Custom Cursor (Desktop Only), Item 3: Text Highlight Reveal on Beat 3 Lede, Item 4: SlotButton Hover on Beat 7 Tooltips, Item 5: Scramble Text on Kickers (+2 more)
...
Cohesion: 0.20
Nodes (9): Apply plan (once you approve), C1. Rewrites (de-fluffed — these violated §12 or were too thin), C2. Keep as-is (already tight & factual — no change), GEO Copy Review — for approval before applying, How I rewrote things (the rules I applied), PART A — The technical GEO layer (highest ROI, currently missing), PART B — Titles (all 90, old → new), PART C — Descriptions (+1 more)
...
Cohesion: 0.25
Nodes (8): 12.1 CSS Custom Easing Curves, 12.2 Scroll Indicator (Alche Pattern), 12.3 Mission Text Mask Reveal, 12.4 Slot Button (Text Swap on Hover), 12.5 Section-Based CSS Gating, 12.6 Staggered Loading Bar, 12.7 Image on Scroll Change (Works Pattern), 12. Code Snippet Library
...
Cohesion: 0.29
Nodes (6): 1. Executive Summary, 9.1 ArtPill's Scanner Loading Sequence, 9.2 Alche's Lottie Loading Sequence, 9. Loading Sequences, Advanced Web Interaction Patterns — Study Report, Table of Contents
...
Cohesion: 0.33
Nodes (4): CLIENT_OVERRIDES, data, EDU_DEFAULTS, ROLE_OVERRIDES

### Community 358 - "Community 358"
Cohesion: 0.33
Nodes (6): 4.1 Character Reveal (ArtPill's GSAP SplitText), 4.2 Text Highlight Reveal (Alche's CSS Mask), 4.3 SlotButton Text Slide-Up (Alche's Hover Effect), 4.4 Scramble Text Effect (Alche), 4.5 Key Typography CSS, 4. Text & Typography Animation

### Community 359 - "Community 359"
Cohesion: 0.33
Nodes (6): 5.1 Persistent Canvas Architecture, 5.2 Alche's Three.js Debug Panel (Tweakpane), 5.3 Section-Driven Context Switching, 5. 3D & WebGL Spatial Experience, 5.4 Performance: WebGL Behind Text, 5.5 Implementation Pattern
...
Cohesion: 0.40
Nodes (5): 13. Appendix: Tooling & Resources, Browser DevTools Workflow, Libraries Detected in the Wild, Performance Budget Recommendations, References

### Community 381 - "Community 381"
Cohesion: 0.40
Nodes (5): 2.1 ArtPill Studio, 2.2 Alche Studio, 2.3 Studio Dialect, 2.4 Vectr, 2. Site-by-Site Architecture Breakdown
...
Cohesion: 0.50
Nodes (4): 10.1 What These Sites Do, 10.2 Alche's Approach, 10.3 Font Loading Strategy, 10. Performance Patterns

### Community 409 - "Community 409"
Cohesion: 0.50
Nodes (4): 6.1 Custom Cursor (ArtPill), 6.2 Magnetic Hover Effect, 6.3 Scanline/Cursor Overlay (ArtPill's Scanner), 6. Cursor & Pointer Interaction

### Community 410 - "Community 410"
Cohesion: 0.50
Nodes (4): 7.1 Swup-Based SPA (Alche), 7.2 Next.js Transitions (ArtPill), 7.3 Section-Based Composition (Alche's Approach), 7. Composition & View Switching

### Community 411 - "Community 411"
Cohesion: 0.50
Nodes (4): 8.1 Works Thumbnail Scroll (Alche), 8.2 News Overlay, 8.3 Image Loading & Progressive Enhancement, 8. Information Overlays & Media
